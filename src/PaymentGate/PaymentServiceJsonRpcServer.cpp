  ...
  , logger(loggerGroup, "PaymentServiceJsonRpcServer")
{
  ...
  handlers.emplace("getTrackingKey", jsonHandler<GetTrackingKey::Request, GetTrackingKey::Response>(std::bind(&PaymentServiceJsonRpcServer::handleGetTrackingKey, this, std::placeholders::_1, std::placeholders::_2)));
...
std::error_code PaymentServiceJsonRpcServer::handleGetTrackingKey(const GetTrackingKey::Request& request, GetTrackingKey::Response& response) {
  return service.getTrackingKey(response.trackingKey);
}
