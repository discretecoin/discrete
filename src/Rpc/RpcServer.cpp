...
void RpcServer::processRequest(const CryptoNote::HttpRequest& request, CryptoNote::HttpResponse& response) {
  ...
  if (url == "/stop_daemon" || labEndpoint) {
...
bool RpcServer::on_swap_lab_outpoint(const COMMAND_RPC_SWAP_LAB_OUTPOINT::request& req, COMMAND_RPC_SWAP_LAB_OUTPOINT::response& res) {
  ...
  m_core.executeLocked([&]() -> std::error_code {
    ...
    if (res.spent_known) {
      ...
      for (const auto& pending : m_core.getPoolTransactions()) {
        for (const auto& input : pending.inputs) {
          Crypto::KeyImage candidate{};
          if (transactionSpendTag(input, chain, candidate) && candidate == tag) res.spent_in_pool = true;
        }
      }
    }
    return {};
