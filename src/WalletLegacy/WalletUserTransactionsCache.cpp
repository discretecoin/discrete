...
TransactionId WalletUserTransactionsCache::addNewTransaction(
  uint64_t amount, uint64_t fee, const std::string& extra, const std::vector<WalletLegacyTransfer>& transfers, uint64_t unlockHeight) {
...
void WalletUserTransactionsCache::updateTransaction(
  TransactionId transactionId, const CryptoNote::Transaction& tx, uint64_t amount, const std::list<TransactionOutputInformation>& usedOutputs, Crypto::SecretKey& reservedKey) {
  ...
  txInfo.secretKey = reservedKey;
  m_unconfirmedTransactions.add(tx, transactionId, amount, usedOutputs, reservedKey);
}
...
std::shared_ptr<WalletLegacyEvent> WalletUserTransactionsCache::onTransactionUpdated(const TransactionInformation& txInfo, int64_t txBalance) {
  ...
  if (id == CryptoNote::WALLET_LEGACY_INVALID_TRANSACTION_ID) {
    ...
    transaction.unlockHeight = txInfo.unlockHeight;
    transaction.secretKey = NULL_SECRET_KEY;
