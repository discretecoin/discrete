...
namespace CryptoNote {
  ...
  class Core : public ICore, public IMinerHandler, public IBlockchainStorageObserver, public ITxPoolObserver {
   public:
     Core(const Currency& currency, i_cryptonote_protocol* pprotocol, Logging::ILogger& logger, System::Dispatcher& dispatcher, uint32_t rejectDeepReorgDepth = 0, bool noBlobs = false);
     ~Core();
     ...
     bool get_block_template_pq(Block& b, const CryptoPQ::KemPublicKey& viewPub, const CryptoPQ::DsaPublicKey& spendPub, Difficulty& diffic, uint32_t& height, const BinaryArray& ex_nonce);
     ...
     virtual bool getBlockDifficulty(uint32_t height, Difficulty& difficulty) override;
     ...
     virtual bool getblockEntry(uint32_t height, uint64_t& block_cumulative_size, Difficulty& difficulty, uint64_t& already_generated_coins, uint64_t& reward, uint64_t& transactions_count, uint64_t& timestamp) override;
     virtual bool getBlockStats(uint32_t startHeight, uint32_t endHeight, std::vector<BlockStatsEntry>& stats) override;
     virtual bool getBlockStats(const std::vector<uint32_t>& heights, std::vector<BlockStatsEntry>& stats) override;
