...
// Recipient PUBLIC keys (view + spend), one per premine tranche. AUTO-GENERATED;
// see docs/GENESIS.md. Defines kGenesisPremineViewPubHex[] /
// kGenesisPremineSpendPubHex[] (GENESIS_PREMINE_TRANCHES entries each).
#include "GenesisPremineKeys.inc"

namespace {

// Domain-separated, deterministic per-tranche seeds. The ML-KEM encapsulation
// message and the output blinding value rho are both derived from the tranche
...
CryptoPQ::Hash256 trancheSeed(const char* domain, std::size_t domainLen, uint32_t i) {
  uint8_t ikm[4];
...
std::array<uint8_t, N> decodeFixedHex(const std::string& hex, const char* what) {
  if (hex.empty()) {
    throw std::runtime_error(std::string("Genesis premine recipient key not provisioned (") +
                             what + "). Run: vanitygen --premine-accounts and rebuild.");
  }
  std::array<uint8_t, N> out{};
  size_t sz = 0;
  if (!Common::fromHex(hex, out.data(), out.size(), sz) || sz != N) {
    throw std::runtime_error(std::string("Genesis premine recipient key has wrong size (") + what + ")");
  }
  return out;
}
...
Transaction buildGenesisPremineCoinbase() {
  ...
  tx.unlockHeight = 0;  // genesis uses PER-OUTPUT unlockHeights for the tranches
  tx.inputs.clear();
  tx.outputs.clear();
  tx.extra.clear();
  ...
  for (uint32_t i = 0; i < GENESIS_PREMINE_TRANCHES; ++i) {
    auto viewPub = decodeFixedHex<CryptoPQ::kKemPublicKeyBytes>(
        kGenesisPremineViewPubHex[i], "viewPub");
    auto spendPub = decodeFixedHex<CryptoPQ::kDsaPublicKeyBytes>(
        kGenesisPremineSpendPubHex[i], "spendPub");

    CryptoPQ::Hash256 kemSeed = trancheSeed(kKemSeedDomain, sizeof(kKemSeedDomain) - 1, i);
    CryptoPQ::Rho      rho     = trancheSeed(kRhoSeedDomain, sizeof(kRhoSeedDomain) - 1, i);
    ...
    CryptoPQ::PqBuiltOutput built = CryptoPQ::buildPqOutput(
        ...
        GENESIS_PREMINE_TRANCHE_ATOMS, rho, /*T=*/0);
    ...
    out.amount = GENESIS_PREMINE_TRANCHE_ATOMS;
    out.unlockHeight = static_cast<uint64_t>(i) * GENESIS_PREMINE_UNLOCK_STEP;
