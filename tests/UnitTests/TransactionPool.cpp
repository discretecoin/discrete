...
// Fusion-transaction pool tests removed: Discrete has no fusion transactions, and
// the tx pool only accepts PQ transactions (classical KeyInput txs are rejected).
// PQ tx-pool behaviour (double-spend / nullifier handling) is covered by the
// Pq* suites (tests/test_pq_validation.cpp, test_pq_chain.cpp).
