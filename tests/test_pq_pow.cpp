...
// SHAKE256 of eight stock/discrete output pairs, generated by the scalar build.
const char kCoreCorpusDigest[] = "6473818860e1a3a4941217b819e7a8e09553f4ac62ae2ce8959d5804f2560d19";
...
TEST(PqPow, DeterministicCoreCorpus) {
  const std::array<uint8_t, 32>& P = discrete_power_memory_personalization();
  yespower_params_t yp{ parameters::DISCRETE_POWER_N, parameters::DISCRETE_POWER_R,
                        P.data(), P.size() };
  yespower_local_t local;
  ASSERT_EQ(yespower_init_local(&local), 0);
  std::vector<uint8_t> outputs;
  outputs.reserve(8 * 2 * sizeof(yespower_binary_t));

  for (uint8_t k = 0; k < 8; ++k) {
    std::array<uint8_t, 64> H{};
    for (size_t i = 0; i < H.size(); ++i) {
      H[i] = static_cast<uint8_t>(i * (11 + 2 * k) + 3 + 17 * k);
    }

    std::array<uint8_t, parameters::DISCRETE_POWER_TAPE_LEN> tape{};
    for (size_t i = 0; i < tape.size(); ++i) {
      tape[i] = static_cast<uint8_t>(i * (5 + 2 * k) + 1 + 29 * k);
    }

    yespower_binary_t stock{};
    yespower_binary_t discrete{};
    ASSERT_EQ(yespower(&local, H.data(), H.size(), &yp, &stock), 0);
    ASSERT_EQ(yespower_discrete(&local, H.data(), H.size(), &yp, tape.data(), &discrete), 0);
    outputs.insert(outputs.end(), stock.uc, stock.uc + sizeof(stock.uc));
    outputs.insert(outputs.end(), discrete.uc, discrete.uc + sizeof(discrete.uc));
  }

  EXPECT_EQ(yespower_free_local(&local), 0);

  std::array<uint8_t, 32> digest{};
  CryptoPQ::shake256(outputs.data(), outputs.size(), digest.data(), digest.size());
  if (kCoreCorpusDigest[0] == '@') {
    std::printf("[ CORPUS ] scalar digest: %s\n", toHex(digest).c_str());
  } else {
    EXPECT_EQ(toHex(digest), kCoreCorpusDigest);
  }
}
