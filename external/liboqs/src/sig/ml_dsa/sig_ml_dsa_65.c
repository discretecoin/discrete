...
extern int pqcrystals_ml_dsa_65_ref_keypair_derand(uint8_t *pk, uint8_t *sk, const uint8_t *seed);  /* Karbo PQ patch */
...
/* Karbo PQ patch — deterministic keygen from a 32-byte seed.
 * See external/liboqs/UPSTREAM.md. The patch routes only to the ref impl
 * because we build with OQS_DIST_BUILD=OFF (no AVX2 path); if that ever
 * changes, mirror the dispatch above and add an AVX2 derand variant too. */
