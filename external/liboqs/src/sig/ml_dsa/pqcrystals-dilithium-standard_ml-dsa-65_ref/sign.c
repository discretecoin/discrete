...
*
* Description: Karbo PQ patch — deterministic variant of crypto_sign_keypair.
*              Sources the SEEDBYTES (=32) seedbuf from the caller instead of
*              randombytes(). All subsequent expansion is deterministic, so
*              this returns a reproducible (pk, sk) pair for a given seed.
*              Required by the PQ Phase-1 spec for per-output spend keys
*              derived from the ML-KEM shared secret.
*
* Arguments:   - uint8_t *pk:         output public key (CRYPTO_PUBLICKEYBYTES)
*              - uint8_t *sk:         output secret key (CRYPTO_SECRETKEYBYTES)
*              - const uint8_t *seed: input seed (SEEDBYTES = 32 bytes)
*
* Returns 0 (success)
**************************************************/
int crypto_sign_keypair_derand(uint8_t *pk, uint8_t *sk, const uint8_t *seed) {
  uint8_t seedbuf[2*SEEDBYTES + CRHBYTES];
  uint8_t tr[TRBYTES];
  const uint8_t *rho, *rhoprime, *key;
  polyvecl mat[K];
  ...
  /* Use caller-provided seed instead of randombytes(). Remainder of the
   * function is byte-for-byte identical to crypto_sign_keypair. */
