...
from .common import sync_directory

def canonical(value):return json.dumps(value,sort_keys=True,separators=(',',':'),allow_nan=False).encode()
def digest(b):return hashlib.sha256(b).hexdigest()

def attempt_evidence(encoded):
    if encoded is None:return None
    if not isinstance(encoded,bytes) or len(encoded)>16384:raise ValueError('bounded attempt evidence required')
    value=json.loads(encoded)
    if type(value) is not dict or canonical(value)!=encoded:raise ValueError('canonical attempt evidence required')
    if '_aad_version' in value and (type(value['_aad_version']) is not int or value['_aad_version']!=1):
        raise ValueError('unsupported attempt evidence authentication')
    return value

def attempt_aad(swap,kind,number,h,txid,evidence):
    identity=[swap,kind,number,h,txid]
    if evidence is not None and evidence.get('_aad_version')==1:identity.append(evidence)
    return canonical(identity)

CLAIM_KINDS=frozenset(('claim','xds-claim','foreign-claim','xds-pair-claim'))
KINDS=CLAIM_KINDS|frozenset(('fund','xds-fund','foreign-fund','refund','xds-refund','foreign-refund','handoff-refund','session'))
...
class Journal:
    ...
    def restore(cls,snapshot,destination,key):
        ...
        try:
            ...
            aead=AESGCM(key)
            for swap,kind,number,h,enc,txid,evidence in copied.execute('SELECT swap,kind,attempt,payload_hash,encrypted,txid,evidence FROM attempts'):
                checked_kind(kind)
                payload=aead.decrypt(enc[:12],enc[12:],attempt_aad(swap,kind,number,h,txid,attempt_evidence(evidence)))
    ...
    def snapshot(self,destination):
        """WAL-consistent encrypted snapshot, published to a NEW reserved path."""
        target=pathlib.Path(destination).resolve()
        if target==self.path.resolve():raise ValueError('distinct snapshot required')
        fd=os.open(target,os.O_CREAT|os.O_EXCL|os.O_WRONLY,0o600);os.close(fd)
        temp=target.with_name(target.name+'.snapshot-'+uuid.uuid4().hex+'.tmp')
        fd=os.open(temp,os.O_CREAT|os.O_EXCL|os.O_WRONLY,0o600);os.close(fd)
        copied=sqlite3.connect(temp,isolation_level=None)
        try:
            self.db.backup(copied)
            copied.execute('PRAGMA journal_mode=DELETE')
            if copied.execute('PRAGMA integrity_check').fetchone()[0]!='ok':raise ValueError('snapshot integrity')
    ...
    def intent(self,swap,kind,attempt=None):
        ...
        h,enc,txid,status,stage,number,exposes,evidence=row;evidence=attempt_evidence(evidence)
        aad=attempt_aad(swap,kind,number,h,txid,evidence)
        ...
        return {'payload':payload,'txid':txid,'status':status,'stage':stage,'attempt':number,
                'exposes':bool(exposes),'evidence':evidence}
    def _insert_attempt(self,swap,kind,number,payload,txid,evidence=None):
        ...
        if evidence is not None:
            if type(evidence) is not dict or '_aad_version' in evidence:raise ValueError('reserved attempt evidence metadata')
            evidence=attempt_evidence(canonical(dict(evidence,_aad_version=1)))
        enc=nonce+self.aead.encrypt(nonce,payload,attempt_aad(swap,kind,number,h,txid,evidence))
    ...
    def prepare(self,swap,kind,payload,txid,exposes=False,admission=None,evidence=None):
        checked_kind(kind)
        ...
        with self.transaction():
            ...
            if old:
                ...
                if evidence is not None and (type(evidence) is not dict or '_aad_version' in evidence or
                  old['evidence']!=dict(evidence,_aad_version=1)):raise ValueError('attempt evidence immutable')
            ...
            self._insert_attempt(swap,kind,0,payload,txid,evidence)
    ...
    def _broadcast_recovered(self,swap,kind,send,admission_supplier,validate):
        ...
        if not self.recovery_required() or kind not in ('xds-claim','xds-refund','foreign-claim','foreign-refund'):
            raise ValueError('qualified recovery settlement only')
        return self._broadcast(swap,kind,send,admission_supplier,validate)
    def _broadcast(self,swap,kind,send,admission_supplier,validate=None,require_fresh=False):
        if kind=='session':raise ValueError('session metadata cannot be broadcast')
        if self.recovery_required() and kind not in ('xds-claim','xds-refund','foreign-claim','foreign-refund'):
            raise ValueError('restored journal permits settlement only')
        with self.transaction():
            ...
            if self.recovery_required():
                ...
                if type(evidence) is not dict or evidence.get('source') not in ('owned-litesvm-recovery-only','verified-chain-settlement-v1'):
            ...
            if intent['exposes'] and (not self.exposed(swap) or require_fresh):
